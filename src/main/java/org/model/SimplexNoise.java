...
/** MODEL: Implementazione del rumore Simplex 2D (public domain, Stefan Gustavson). */
public class SimplexNoise {
    ...
    static { for (int i=0;i<512;i++) { perm[i]=p[i&255]; permMod12[i]=(short)(perm[i]%12); } }
    ...
    public static double noise(double xin, double yin) {
        ...
        double x1=x0-i1+G2,y1=y0-j1+G2,x2=x0-1+2*G2,y2=y0-1+2*G2;
        int ii=i&255,jj=j&255;
        int gi0=permMod12[ii+perm[jj]],gi1=permMod12[ii+i1+perm[jj+j1]],gi2=permMod12[ii+1+perm[jj+1]];
        double n0,n1,n2;
        double t0=0.5-x0*x0-y0*y0; n0 = t0<0?0:((t0*=t0)*t0*dot(grad3[gi0],x0,y0));
        double t1=0.5-x1*x1-y1*y1; n1 = t1<0?0:((t1*=t1)*t1*dot(grad3[gi1],x1,y1));
        double t2=0.5-x2*x2-y2*y2; n2 = t2<0?0:((t2*=t2)*t2*dot(grad3[gi2],x2,y2));
